...
function broadcastToCall(callId, ev, evData) {
	if (!callSockets[callId]) {
		return;
	}

	for (const socketId in callSockets[callId]) {
		const s = callSockets[callId][socketId];
...
function initializeClient(socket, client, token, lastMessage, openChannel) {
	...
	socket.on("disconnect", function () {
		if (socket.calls) {
			for (const call of socket.calls) {
				broadcastToCall(call.callId, "remove-peer", {peerId: socket.id});
			}
		}
	...
	socket.on("call", ({callId, channelName, nick}) => {
		if (!callSockets[callId]) {
			callSockets[callId] = {};
		}

		if (callSockets[callId][socket.id]) {
			return;
		}

		broadcastToCall(callId, "add-peer", {callId, peerId: socket.id});
		...
		socket.calls.push({nick, callId});
		callSockets[callId][socket.id] = socket;

		const users = [];
		for (const socketId in callSockets[callId]) {
			users.push(callSockets[callId][socketId].nick);
		}

		socket.emit("call-started", {callId, channelName, nick, users});
	});
	...
	socket.on("leave-call", (callId) => {
		...
		delete callSockets[callId][socket.id];

		broadcastToCall(callId, "remove-peer", {peerId: socket.id});
	});

	socket.on("relay-ice-candidate", (config) => {
		broadcastToCall(config.callId, "peer-ice-candidate", config);
	});

	socket.on("relay-session-description", (config) => {
		broadcastToCall(config.callId, "peer-session-description", config);
	});
...
async function performAuthentication(data) {
	...
	data.user = `${data.user}_${new Date().getTime()}`;

	client = await manager.findClient(data.user);
	client = new Client(manager, data.user, client);
	...
	const success = await manager.login(client, data.user, data.password);
